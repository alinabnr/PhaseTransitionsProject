...
\section{Metropolis Algorithm for the 2D Ising model}
...
\subsection{Single-Spin Metropolis algorithm (checkerboard) on 2D Ising Model}
...
\section{Wolff Algorithm for the 2D Ising Model}
...
As we saw for the Metropolis algorithm, to quantify critical slowing down we compute the autocorrelation function of an observable \(A\) (for instance the magnetization \(M\), the energy or the susceptibility):
\[
C_A(t) = \langle A_{s} A_{s+t}\rangle - \langle A\rangle^2,
\]
and the normalized autocorrelation (autocorrelation coefficient)
...
with the convenient scalar measure of the memory of the Markov chain i.e. the
\emph{integrated autocorrelation time}
\[
\tau_{\mathrm{int},A} = \frac{1}{2} + \sum_{t=1}^{T_{\max}} \rho_A(t),
\]
...
As we saw previously, at criticality finite-size scaling for relaxational dynamics we can fit \(\log \tau_c\) versus \(\log L\) to extract \(z\):
\[
\log \tau_c = z \log L + \text{const}.
\]
