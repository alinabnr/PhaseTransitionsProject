...
To estimate statistical uncertainties we recommend the blocking (binning) method: group consecutive measurements into blocks of size \(B\) (with \(B\gg \tau_{\mathrm{int}}\)), compute the block means and use the sample variance of block means to estimate the variance of the mean. The block size should be increased until the estimated error stabilizes.