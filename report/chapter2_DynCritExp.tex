...
\begin{equation}
    \chi = \frac{\partial \langle m \rangle}{\partial h} \Big|_{h=0} = \beta (\langle m^2 \rangle - \langle m \rangle^2).
\end{equation}

By the fluctuation-dissipation theorem, this equals to
\begin{equation}
...
- Explain that the Metropolis and Wolff algorithms represent distinct stochastic dynamics (local vs. cluster).

- Describe briefly the nature of updates each performs.

2.3 Dynamic Critical Phenomena


Here is where you derive the concept of dynamic critical exponents.


- Near \(T_c\), the correlation length diverges as \(\xi \sim |T - T_c|^{-\nu}\).

- Introduce time correlation functions, such as

\(C_X(t) = \langle X(0) X(t) \rangle - \langle X \rangle^2\).

- Define the autocorrelation time \(\tau_X\) via the exponential decay of \(C_X(t)\):

\(C_X(t) \sim e^{-t/\tau_X}\).

- State the scaling hypothesis:

\(\tau_X \sim \xi^z\), where \(z\) is the dynamic critical exponent.

- Combining both gives the divergence near criticality:

\(\tau_X \sim |T - T_c|^{-z\nu}\).

- Explain qualitatively that \(z\) depends on the update dynamics (hence on the algorithm).

This section should include equation derivations, reasoning, and physical interpretation.